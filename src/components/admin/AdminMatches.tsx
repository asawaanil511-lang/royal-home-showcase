...
import { Plus, CheckCircle, Trash2, X, Edit2, Save } from "lucide-react";
...
const AdminMatches = () => {
  ...
  const handleEdit = (m: DBMatch) => {
    setEditingId(m.id);
    setEditData({ team_a_name: m.team_a_name, team_b_name: m.team_b_name, odds_a: m.odds_a, odds_b: m.odds_b, match_date: m.match_date.slice(0, 16) });
  };
  ...
  const statusBadge = (status: string) => {
    const cls = status === "live" ? "bg-destructive/10 text-destructive" :
      status === "closed" ? "bg-muted text-muted-foreground" :
      status === "cancelled" ? "bg-[hsl(var(--neon-purple))]/10 text-[hsl(var(--neon-purple))]" :
      "bg-primary/10 text-primary";
    return <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${cls}`}>{status.toUpperCase()}</span>;
  };
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
            ...
            {editingId === m.id ? (
              <div className="space-y-3">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <Input type="datetime-local" value={editData.match_date || ""} onChange={(e) => setEditData({ ...editData, match_date: e.target.value })} className="bg-secondary border-border sm:col-span-2" />
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {m.status === "live" && !m.winner && (
                    <>
                      ...
                      <Button size="sm" variant="outline" className="text-xs border-[hsl(var(--neon-purple))]/30 text-[hsl(var(--neon-purple))]" onClick={() => handleCancel(m.id)}>
        ...
        {matches.length === 0 && <p className="text-center text-muted-foreground py-12">No matches yet.</p>}
      </div>
    </div>
  );
};
