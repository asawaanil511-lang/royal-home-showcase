...
import { Crown, LayoutDashboard, Trophy, Users, Receipt, UserCog, Megaphone, Loader2 } from "lucide-react";
...
const Owner = () => {
  ...
  useEffect(() => {
    ...
    const checkRole = async () => {
      const { data } = await (supabase as any).from("user_roles").select("role").eq("user_id", user.id).eq("role", "owner").maybeSingle();
  ...
  if (authLoading || checking || !isOwner) {
    return (
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.div layoutId="owner-tab-indicator"
